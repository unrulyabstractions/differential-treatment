...
Ported from queering-nlp-bias `src/common/base_schema.py`, with the torch
dependency replaced by numpy handling (this pipeline never stores tensors).
"""
...
def _canon(
    ...
    """
    # Convert numpy scalars/arrays to plain Python before canonicalizing
    if isinstance(obj, np.ndarray):
        return _canon(obj.tolist(), places, max_list_length, max_string_length)
    if isinstance(obj, np.bool_):
        return bool(obj)
    if isinstance(obj, np.floating):
        return _canon(float(obj), places, max_list_length, max_string_length)
    ...
    if is_dataclass(obj):
        ...
        # Apply _to_dict_hook if available (for derived-field expansion, etc.)
    ...
    def _convert_value(cls, val, field_type):
        ...
        if is_dataclass(field_type) and hasattr(field_type, "from_dict"):
            ...
            if isinstance(val, list):
                # Some dataclasses support parsing from lists
                return field_type.from_dict(val)
